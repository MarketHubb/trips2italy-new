...
function gravity_form_to_tailwind_exact($form)
{
    ...
    $output .= '<form data-form-id="' . $form['id'] . '" id="gform_' . $form['id'] . '" class="gform_wrapper gravity-theme px-4 sm:px-8 md:px-10 space-y-10 md:space-y-16 divide-y divide-gray-900/10 bg-transparent" method="post" enctype="multipart/form-data">';
    ...
    if (!empty($form['pagination'])) {
        ...
        foreach ($form['pagination']['pages'] as $page_index => $page_info) {
            ...
            $stylized_formatted = '<span class="stylized pl-1 text-[130%] text-brand-600">' . $stylized_word . '</span>';
            $formatted_title = '';
            ...
            $output .= '<h2 class="tracking-tight font-base font-normal leading-7 text-brand-600 md:pt-6">' . $formatted_title . '</h2>';
            $output .= '</div>';
            $output .= '<p class="mt-1 text-base leading-6 text-gray-700 md:pr-12">' . esc_html($description) . '</p>';
            $output .= '</div>';

            $output .= '<div class="p-2 sm:p-6 bg-white shadow-md ring-1 ring-gray-300 rounded-xl md:col-span-2">';
...
function render_field_exact($field, $form)
{
    ...
    if ($is_hidden) {
        ...
        $input_class_array = get_input_class_array($field);
        $is_required = isset($field['isRequired']) && $field['isRequired'];
        $input_class = ' peer block w-full rounded-md border-0 py-1.5 px-3 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-brand-500 sm:text-base sm:leading-6 placeholder:italic placeholder:text-slate-400 ';
        // $input_class_array = explode(' ', $field['cssClass']);
        ...
        switch ($field['type']) {
            ...
            case 'checkbox':
            case 'radio':
                $output .= get_checkbox_radio_input_open($field, $input_class_array);
...
function get_checkbox_radio_input_open($field, $input_class_array)
{
    ...
    if (in_array('row-cards', $input_class_array)) {
        ...
        $output .= '<div class="mt-6 grid ' . $sm_grid_cols . ' sm:grid-cols-3 gap-4 auto-rows-fr">';
...
function get_attributes_for_radio_and_checkboxes($form, $field, $index)
{
    ...
    return [
        ...
        'name' => $field['type'] === 'radio' ? 'input_' . $field['id'] : 'input_' . $field['inputs'][$index]['id'],
        'input_index' => $field['type'] === 'radio' ? $index : $index + 1,
...
function render_checkbox_input($field, $form)
{
    ...
    foreach ($field[$field_key] as $index => $input) {
        $input_attributes = get_attributes_for_radio_and_checkboxes($form, $field, $index);

        $output .= '<div class="relative">'; // Wrapper to maintain layout
        $output .= '<input type="' . $field['type'] . '" name="' . $input_attributes['name'] . '" id="choice_' . $input_attributes['id'] . '" value="' . $input_attributes['value'];
        $output .= '" class="sr-only peer" data-index="' . $index . '" ' . $input_attributes['required'] . '>';
        $output .= '<label for="choice_' . $input_attributes['id'] . '" class="flex cursor-pointer h-full rounded-lg border text-gray-600 font-medium bg-white px-4 py-2 sm:p-4 shadow-sm focus:outline-none peer-checked:ring-2 peer-checked:ring-brand-500 peer-checked:bg-brand-100/30 peer-checked:text-gray-900">';
        $output .= '<span class="flex flex-1 items-center sm:items-start">';
        $output .= '<span class="flex flex-col">';
        $output .= '<span class="block text-sm sm:text-base tracking-normal leading-normal pr-5">' . $input_attributes['label_parts'][0] . '</span>';
        if (isset($input_attributes['label_parts'][1])) {
            $output .= '<span class="mt-1 flex font-normal items-center text-sm text-gray-500">' . $input_attributes['label_parts'][1] . '</span>';
        }
        $output .= '</span>';
        $output .= '</span>';
        $output .= '</label>';
        $output .= '<svg class="absolute right-[10px] top-[25%] sm:top-[18px] h-5 w-5 text-brand-500 hidden peer-checked:block pointer-events-none" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">';
...
function render_hidden_field($form, $field)
{
    ...
    if ($field['id'] === 27) {
        ...
        $output .= '<input type="' . $field['type'] . '" name="input_' . $field['id'] . '" value="' . $referring_url . '">';
    } else {
        if ($field['type'] === 'checkbox' || $field['type'] === 'radio' && !empty($field['choices'])) {
            foreach ($field['choices'] as $index => $input) {
                $input_attributes = get_attributes_for_radio_and_checkboxes($form, $field, $index);
                $checked = $input_attributes['selected'] ? ' checked' : '';
                $output .= '<input class="hidden" type="' . $field['type'] . '" name="' . $input_attributes['name'] . '" id="choice_' . $input_attributes['id'] . '" value="' . $input_attributes['value'] . '"' . $checked . '>';
            }
...
function render_standard_input($field, $choice, $choice_id, $index, $label_parts)
{
    ...
    $output .= '<input id="' . $choice_id . '" name="input_' . $field['id'] . '" type="' . $field['type'] . '" value="' . esc_attr($choice['value']) . '" class="h-4 w-4 border-gray-300 text-brand-500 focus:ring-brand-500">';
    $output .= '</div>';
...
function get_form_submit($form)
{
    ...
    $output .= '<input type="hidden" name="action" value="submit_custom_gravity_form">';
    $output .= '<input type="hidden" name="nonce" value="' . esc_attr(wp_create_nonce('submit_custom_gravity_form')) . '">';
    $output .= '<input type="submit" id="gform_submit_button_' . $form['id'] . '" class="w-full rounded-md border border-transparent bg-secondary-500 px-4 py-3 text-base font-medium text-white shadow-sm hover:bg-secondary-600 focus:outline-none focus:ring-2 focus:ring-secondary-500 focus:ring-offset-2 focus:ring-offset-gray-50" value="' . $form['button']['text'] . '">';
    $output .= '</div></div>';
    return $output;
}

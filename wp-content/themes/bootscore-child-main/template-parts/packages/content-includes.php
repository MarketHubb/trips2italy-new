<div class="py-24 sm:py-32">
	<div class="mx-auto max-w-7xl px-6 lg:px-8">
		<div class="mx-auto max-w-2xl lg:text-center">
			<h2 class="stylized text-xl sm:text-2xl lg:text-3xl font-semibold leading-7 text-brand-500">What's included
			</h2>
			<?php
		...
		<div class="mx-auto mt-16 max-w-2xl sm:mt-20 lg:mt-24 lg:max-w-none">
			<dl class="grid max-w-xl grid-cols-1 gap-x-8 gap-y-16 lg:max-w-none lg:grid-cols-1">
				...
				<?php
				$content_fields = [
					'li_class' => ' snap-item rounded-xl w-9/12 lg:w-full flex-shrink-0 snap-center bg-brand-100 ring-1 ring-brand-100 lg:opacity-100 transition-all duration-300 ease-in-out shadow-md ',
					'item_div_classes' => ' bs-blur flex flex-col lg:flex-row lg:gap-x-4 h-full overflow-hidden rounded-xl p-4 lg:px-6 transition-all duration-300 ease-in-out transform ',
					'heading_class' => ' d-inline-block fw-bolder text-uppercase mt-3 mb-0 text-xl md:text-[1.25rem] '
				];
				$featured_field = get_field('featured_includes');

				if (!empty($featured_field)) : ?>
					<?php
					foreach ($featured_field as $field) :
						$content_fields['content'][] = [
							'heading' => $field['callout'],
					...
					<?php if (!empty($content_fields['content'])): ?>

						<?php echo get_scroll_panels($content_fields); ?>

						<?php get_template_part('template-parts/packages/content', 'included'); ?>
				...
				<?php if (have_rows('featured_includes')): ?>
					<?php while (have_rows('featured_includes')) : the_row(); ?>
						...
						<div class="hidden flex flex-col justify-center mx-auto">
							<dt class="flex items-center gap-x-3 text-base font-semibold leading-7 text-gray-900 relative">
								<?php if (get_sub_field('icon')) { ?>
									<div class="inline md:absolute md:-left-10 text-brand-500" viewBox="0 0 20 20"
										fill="currentColor" aria-hidden="true">
										<i class="<?php echo get_sub_field('icon'); ?> fa-xl fa-light"></i>
									</div>
								<?php } ?>

								<span class="text-brand-700 tracking-tight text-xl md:text-2xl">
									<?php echo ucwords(get_sub_field('callout')); ?>
								</span>
							</dt>
							<dd class="mt-4 flex flex-auto flex-col text-lg leading-7 text-gray-600">
								<p class="flex-auto text-xl">
									<?php echo ucfirst(get_sub_field('description')); ?>
								</p>
							</dd>
						</div>
		...
		<div
			...
			<dl class="col-span-2 grid grid-cols-1 gap-x-8 gap-y-10 text-base leading-7 text-gray-600 lg:gap-y-16">
				...
				<div class="relative pl-9">
					...
					<dd class="mt-2">In sit qui aliquid deleniti et. Ad nobis sunt omnis. Quo sapiente dicta laboriosam.
					</dd>
				</div>
				<div class="relative pl-9">
					<dt class="font-semibold text-gray-900">

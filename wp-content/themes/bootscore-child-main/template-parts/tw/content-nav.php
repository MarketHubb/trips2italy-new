<header class="bg-brandMedium fixed top-0 left-0 right-0 w-full z-50 shadow-md" id="tw-nav-primary">
   <nav class="mx-auto flex max-w-7xl items-center justify-between !px-6 lg:px-8" aria-label="Global">
      ...
      <div class="hidden lg:flex lg:justify-center lg:items-center pt-3 pb-2  lg:gap-x-12">
         ...
         foreach ($nav_fields as $nav_field) {
            $nav_active_class = get_nav_active_class(get_queried_object(),$nav_field['page']->ID, $nav_field['link_text']) ? ' drop-shadow-[0_-5px_15px_rgba(255,255,255,1)] opacity-100 ' : '  ';
            $base_classes = 'group flex flex-col items-center text-sm font-semibold leading-6 text-white hover:opacity-100 opacity-60 ';
            // $active_class = (isset($current_parent_id) && $current_parent_id === $nav_field['page']->ID) ? 'active ' : '';
            // $desktop_nav_links .= '<a data-id="' . get_the_id() . '" data-parent-id="' . $current_parent_id . '"  href="' .  get_permalink($nav_field['page']->ID) . '" class="' . $base_classes . $active_class . '">';

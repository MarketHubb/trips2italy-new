...
<?php
$text_color = $args['content']['text_color'] ?? 'Dark';
$heading_color_class = heading_color_class($text_color);
$subheading_color_class = subheading_color_class($text_color);
$description_color_class = get_description_color_class($text_color);
?>
<!-- Hero -->
<div class="relative">
    <?php if (!empty($args['content']['image_desktop']) || !empty($args['content']['image_mobile'])): ?>
        <!-- Background images container -->
        <div class="absolute inset-0 w-full h-full">
            <?php if (!empty($args['content']['image_desktop'])): ?>
                <!-- Desktop background image - hidden on small screens, visible from md up -->
                <div class="hidden md:block absolute inset-0 w-full h-full">
                    <img
                        src="<?php echo esc_url($args['content']['image_desktop']['url']); ?>"
                        alt=""
                        class="w-full h-full object-cover" />
                </div>
            <?php endif; ?>

            <?php if (!empty($args['content']['image_mobile'])): ?>
                <!-- Mobile background image - visible on small screens, hidden from md up -->
                <div class="block md:hidden absolute inset-0 w-full h-full">
                    <img
                        src="<?php echo esc_url($args['content']['image_mobile']['url']); ?>"
                        alt=""
                        class="w-full h-full object-cover" />
                </div>
            <?php endif; ?>
            ...
            <!-- Optional overlay -->
            <?php if ($args['content']['overlay'] !== 'None') {
                $bg_color_class = $args['content']['overlay'] === 'Dark'
                    ? ' bg-black '
                    : ' bg-white ';
                $overlay_opacity_class = 'opacity-' . $args['content']['opacity'];
            ?>
                <div class="absolute inset-0 <?php echo $bg_color_class; ?> <?php echo $overlay_opacity_class; ?>"></div>
            <?php } ?>
        </div>
    <?php endif; ?>
    <div class="relative z-10 max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto">
        <!-- Grid -->
        <div class="grid items-center md:grid-cols-2 gap-8 lg:gap-12">
            <div>
                <?php if ($args['content']['callout']): ?>
                    <p class="inline-block text-sm font-semibold uppercase tracking-normal md:tracking-widest <?php echo $heading_color_class; ?>">
                        <?php echo $args['content']['callout']; ?>
                    </p>
                <?php endif; ?>
                <!-- Title -->
                <div class="mt-4 md:mb-12 max-w-2xl">
                    <?php if ($args['content']['heading']): ?>
                        ...
                        <h1 class="mb-4 font-semibold text-gray-800 text-4xl lg:text-5xl text-balance text-pretty <?php echo $heading_color_class; ?>">
                            <?php echo $args['content']['heading'] ?>
                        </h1>
                    <?php endif; ?>

                    <?php if ($args['content']['description']): ?>
                        <p class="font-medium <?php echo $description_color_class; ?>">
                ...
                <!-- Blockquote -->
                <blockquote class="hidden md:hidden relative max-w-sm">
            ...
            <div>
                ...
                if (function_exists('gravity_form')) {
                    // This call returns the form markup as a string, which we then echo manually.
                    // $form_html = gravity_form(
                    //     15,         // The Form ID
                    //     false,      // Hide title
                    //     false,      // Hide description
                    //     false,      // Do not display if inactive
                    //     null,       // No special field values
                    //     true,       // Use AJAX
                    //     0,          // Tab index
                    //     false       // Return the form instead of echoing
                    // );

                    // Now we can echo or manipulate $form_html:
                    // echo $form_html;
                }
        ...
        <!-- Clients Section -->
        <div class="mt-6 md:mt-12 py-3 flex items-center text-sm text-gray-800 gap-x-1.5 after:flex-1 after:border-t after:border-gray-200 after:ms-6 hidden">
            <span class="font-semibold bg-clip-text bg-gradient-to-l from-blue-600 to-violet-500 text-transparent">50,000</span>
            individuals and companies trust Preline
        </div>
        <!-- Clients -->
        <div class="flex flex-wrap gap-x-6 sm:gap-x-12 lg:gap-x-24 hidden">
            <svg class="py-3 lg:py-5 w-16 h-auto md:w-20 lg:w-24 text-gray-500" enable-background="new 0 0 2499 614" viewBox="0 0 2499 614" xmlns="http://www.w3.org/2000/svg">
            ...
            <svg class="py-3 lg:py-5 w-16 h-auto md:w-20 lg:w-24 text-gray-500" xmlns="http://www.w3.org/2000/svg" viewBox="-4.126838974812941 0.900767442746961 939.436838974813 230.18142889845947" width="2500" height="607">
            ...
            <svg class="py-3 lg:py-5 w-16 h-auto md:w-20 lg:w-24 text-gray-500" xmlns="http://www.w3.org/2000/svg" x="0" y="0" viewBox="0 0 88 22" xml:space="preserve" enable-background="new 0 0 88 22">
            ...
            <svg class="py-3 lg:py-5 w-16 h-auto md:w-20 lg:w-24 text-gray-500" fill="none" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 32">
                <g clip-path="url(#a)">
                    <path d="M46.6 11c.5-.7 1-1.3 1.9-1.7.7-.5 1.6-.7 2.6-.7v3.9h-1c-1.1 0-2 .2-2.6.8-.6.5-1 1.5-1 2.8v7.3H43V8.8h3.7v2.3ZM53 16c0-1.4.2-2.7.8-3.9a6.6 6.6 0 0 1 5.9-3.6 6 6 0 0 1 4.9 2.4v-2h3.7v14.5h-3.7v-2.1c-.5.6-1.2 1.2-2 1.7-.9.4-1.8.6-3 .6a6.4 6.4 0 0 1-5.8-3.6 8.7 8.7 0 0 1-.9-4Zm11.6.1c0-.9-.2-1.7-.6-2.3-.3-.6-.8-1.1-1.4-1.5-.6-.3-1.3-.5-2-.5a3.8 3.8 0 0 0-3.3 2c-.4.6-.5 1.4-.5 2.2 0 1 .1 1.7.5 2.4a3.8 3.8 0 0 0 3.4 2 4 4 0 0 0 1.9-.5c.6-.4 1-.9 1.4-1.5.4-.6.6-1.4.6-2.3ZM86.4 8.8l-9.2 21.5h-4l3.3-7.3-6-14.2h4.2L78.5 19l3.9-10.2h4ZM95.2 23.6a8 8 0 0 1-3.8-.9 7 7 0 0 1-2.7-2.7 8 8 0 0 1-1-3.9c0-1.5.3-2.8 1-4a7 7 0 0 1 2.8-2.6 8 8 0 0 1 3.8-1 8 8 0 0 1 3.9 1 7.6 7.6 0 0 1 3.7 6.6c0 1.5-.3 2.8-1 4a7 7 0 0 1-2.8 2.6c-1.2.6-2.4 1-3.9 1Zm0-3.2a3.6 3.6 0 0 0 3.3-2c.4-.6.6-1.4.6-2.3 0-1.4-.4-2.4-1.1-3.2a3.6 3.6 0 0 0-2.7-1.1c-1.1 0-2 .4-2.7 1.1-.7.8-1.1 1.8-1.1 3.2 0 1.4.4 2.5 1 3.2.8.7 1.6 1.1 2.7 1.1ZM114.1 8.6c1.8 0 3.2.6 4.3 1.7 1 1 1.6 2.6 1.6 4.5v8.6h-3.7v-8c0-1.2-.3-2.1-1-2.7-.5-.7-1.3-1-2.3-1-1 0-1.9.3-2.5 1-.6.6-.8 1.5-.8 2.6v8h-3.8V8.9h3.8v1.8a5 5 0 0 1 1.8-1.4 6 6 0 0 1 2.6-.6ZM25.1 0H7C3 0 0 3 0 6.9V25C0 29 3 32 6.9 32H25c3.8 0 6.9-3 6.9-6.9V7C32 3 29 0 25.1 0Z" fill="currentColor" />
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M12.7 12a2.5 2.5 0 1 0 0-5.1 2.5 2.5 0 0 0 0 5Zm4-5.1a6.1 6.1 0 1 1 0 12.1V7Zm0 12.1 6.2 6.1H9V11.4l7.7 7.6Z" fill="currentColor" class="fill-white" />
                </g>
            </svg>

            <svg class="py-3 lg:py-5 w-16 h-auto md:w-20 lg:w-24 text-gray-500" xmlns="http://www.w3.org/2000/svg" width="2500" height="1036" viewBox="0 -6.166 294.398 121.975">
